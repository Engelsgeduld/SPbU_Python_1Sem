...
def xmatrix(matrixs, id1, id2):
    ...
    new_matrix = [
        [
            sum(a * b for a, b in zip(row_numer1, col_numer2))
            for col_numer2 in zip(*matrixs[id2])
        ]
        for row_numer1 in matrixs[id1]
    ]
    return new_matrix
...
def sum_matrix(matrixs, id1, id2):
    new_matrix = [
        [matrixs[id1][j][i] + matrixs[id2][j][i] for i in range(len(matrixs[id1][0]))]
        for j in range(len(matrixs[id1]))
    ]
    print(new_matrix)
    return new_matrix
...
def transpose(matrixs, id):
    new_matrix = [
        [matrixs[id][j][i] for j in range(len(matrixs[id]))]
        for i in range(len(matrixs[id][0]))
    ]
    return new_matrix
...
def delete_vector(vectors):
    if len(vectors) == 0:
        print(
            "В списке нет векторов, внесите вектора, чтобы иметь возможность их удалить"
        )
        return None
    see_vectors(vectors)
    chosen = int(input())
...
def angle(vectors, id1, id2):
    ...
    result = (
        acos(
            scalar(vectors, id1, id2)
            / (len_vector(vectors, id1) * len_vector(vectors, id2))
        )
        * 57.2958
    )
    return result
...
def vectors_menu(vectors):
    ...
    match user_input:
        ...
        case "5":
            ...
            print(
                f"Скалярное произведение векторов {imp[0] + 1} и {imp[1] + 1}: {scalar(vectors, imp[0], imp[1])}"
            )
        case "6":
            print("Введите номера 2 векторов через пробел")
            imp = [int(a) - 1 for a in input().split(" ")]
            print(
                f"Угол между векторами {imp[0] + 1} и {imp[1] + 1} в градусах{angle(vectors, imp[0], imp[1])}"
            )
        case "0":
            exit()
    return vectors
...
def matrixs_menu(matrixs):
    ...
    match user_input:
        case "1":
            print(
                "Введите строки матрицы через запятую, значения через пробел. Пример: 1 2 3,3 2 1"
            )
        ...
        case "3":
            ...
            numer = int(input())
            print(
                f"Результат транспонирования матрицы {numer}"
                + print_matrix(transpose(matrixs, numer - 1))
            )
        case "4":
            see_matrixs(matrixs)
            ...
            print(
                f"Результат сложения матриц {numer1} и {numer2}"
                + print_matrix(sum_matrix(matrixs, numer1 - 1, numer2 - 1))
            )
        case "5":
            see_matrixs(matrixs)
            ...
            numer1, numer2 = map(int, input().split(" "))
            print(
                f"Результат умножения матриц {numer1} и {numer2}"
                + print_matrix(xmatrix(matrixs, numer1 - 1, numer2 - 1))
            )
        case "0":
            exit()
    return matrixs
