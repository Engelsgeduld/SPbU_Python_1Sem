...
def main():
    ...
    if user_input[1] == "wc":
        res = (
            " ".join([a for a in user_input[1:]])
            + " "
            + " ".join(map(str, wc(user_input[-1], user_input[-2])))
        )
        print(res)
    if len(user_input) == 5:
        ...
        if user_input[1] == "head":
            res = " ".join(
                [a for a in user_input[1:]]
                + [str(head(user_input[-2], user_input[-1], user_input[-3]))]
            )
            print(res)
        if user_input[1] == "tail":
            res = " ".join(
                [a for a in user_input[1:]]
                + [str(tail(user_input[-3], user_input[-1], int(user_input[-2])))]
            )
            print(res)
    elif user_input[1] == "head":
        print(
            "".join([a for a in user_input[1:]] + [str(head(10, user_input[-1], "-n"))])
        )
    elif user_input[1] == "tail":
        print(
            "".join([a for a in user_input[1:]] + [str(tail("-n", user_input[-1], 10))])
        )
